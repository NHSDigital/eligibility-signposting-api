...
import eligibility_signposting_api.wrapper as wrapper
...
def test_validate_query_params_conditions(conditions_input, expected_result, expected_log_msg, captured_log):
    ...
    if not expected_result:
        assert any(
            (record.levelname == "ERROR" and expected_log_msg in record.message)
...
@pytest.mark.parametrize("category_input, expected_result, expected_log_msg", [
...
@pytest.mark.parametrize("include_actions_input, expected_result, expected_log_msg", [
    ("Y", True, None),
    ("N", True, None),
    ("y", True, None),
    ("n", True, None),
    ...
    ("TRUE", False, "Invalid include_actions query param: 'TRUE'"),
    ("YES", False, "Invalid include_actions query param: 'YES'"),
    ("0", False, "Invalid include_actions query param: '0'"),
    ("1", False, "Invalid include_actions query param: '1'"),
    ("", False, "Invalid include_actions query param: ''"),
    (" ", False, "Invalid include_actions query param: ' '"),
])
...
def test_validate_query_params_mixed_valid_invalid_conditions_fail_first(captured_log):
    params = {
        "conditions": "VALID_COND,INVALID!,ANOTHER_VALID",
        "category": "SCREENING",
        "includeActions": "N"
    }
    with captured_log.at_level(logging.ERROR):
    ...
    assert any(
        (record.levelname == "ERROR" and
         "Invalid condition query param: " in record.message)
        for record in captured_log.records
    )
...
def test_validate_query_params_valid_conditions_invalid_category_fail_second(captured_log):
    ...
    assert any(
        (record.levelname == "ERROR" and
         "Invalid category query param: " in record.message)
        for record in captured_log.records
    )
    error_logs = [r for r in captured_log.records if r.levelname == "ERROR"]
    assert len(error_logs) == 1
...
def test_validate_query_params_valid_conditions_category_invalid_actions_fail_third(captured_log):
    ...
    assert any(
        (record.levelname == "ERROR" and
         "Invalid include_actions query param: " in record.message)
        for record in captured_log.records
    )
    error_logs = [r for r in captured_log.records if r.levelname == "ERROR"]
    assert len(error_logs) == 1
